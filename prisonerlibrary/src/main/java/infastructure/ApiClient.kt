...
open class ApiClient(val baseUrl: String, val sessionId: String = "") {
    companion object {
    ...
    protected inline fun <reified T : Any?> request(
        ...
        val headers = if (sessionId != "") {
            val authHeader = mapOf("SessionId" to sessionId)
            requestConfig.headers + defaultHeaders + authHeader
        } else {
        ...
        val accept = (headers[Accept] as String).substringBefore(";").lowercase()
